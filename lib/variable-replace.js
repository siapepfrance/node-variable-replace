...
module.exports = function(options, callback) {
	if (!options || !options.source || !(options.dataSource || options.inlineData)) {
		throw new Error("Usage: require('variable-replace')({ source: 'source/path', dest: 'dest/path', dataSource: 'data.json', inlineData: {var1 : 'val1', var2 : 'val2' } })");
	}
	...
	function resolveDestFile(sourcePattern, sourceFile, callback) {
		const dest = options.dest != null ? options.dest : sourceFile;
		fs.stat(dest, function(err, stats) {
			if (!err && stats.isFile()) {
				callback(dest);
			} else if((!err && stats.isDirectory()) || options.source.length > 1 || glob.hasMagic(sourcePattern) || /^.*[\\\/]$/.test(dest)) {
				// dest is a directory
				...
				callback(path.resolve(dest, path.relative(basePath, sourceFile)));
			} else {
				callback(dest);
			}
		});
